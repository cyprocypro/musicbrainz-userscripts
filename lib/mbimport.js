...
var MBImport = (function() {
    ...
    function fnBuildSearchLink(release) {
        ...
        return '<a class="musicbrainz_import" href="//db.musiculate.com/search?' + url_params.join('&') + '">Search in MusicBrainz</a>';
    }

    // compute HTML of search button
    function fnBuildSearchButton(release) {
        var parameters = searchParams(release);
        var html = '<form class="musicbrainz_import musicbrainz_import_search" action="//db.musiculate.com/search" method="get" target="_blank" accept-charset="UTF-8" charset="' + document.characterSet + '">';
        parameters.forEach(function(parameter) {
          var value = parameter.value + "";
    ...
    function fnSearchUrlFor(type, what) {
      ...
      var params = [
        ...
        return '//db.musiculate.com/search?' + params.join('&');
    }
    ...
    function fnBuildFormHTML(parameters) {
        ...
        // Build form
        var innerHTML = '<form class="musicbrainz_import musicbrainz_import_add" action="//db.musiculate.com/release/add" method="post" target="_blank" accept-charset="UTF-8" charset="' + document.characterSet + '">';
        parameters.forEach(function(parameter) {
            var value = parameter.value + "";
            innerHTML += "<input type='hidden' value='" + value.replace(/'/g,"&apos;") + "' name='" + parameter.name + "'/>";
        });

        innerHTML += '<button type="submit" title="Import this release into MusicBrainz (open a new tab)"><img src="//db.musiculate.com/favicon.ico" /><span>Import into MB</span></button>';
        innerHTML += '</form>';
